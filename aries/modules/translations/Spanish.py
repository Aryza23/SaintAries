...
SpanishStrings = {
    ...
    "The original sender, {}, has an ID of `{}`.\nThe forwarder, {}, has an ID of `{}`.": "El remitente, {}, tiene el ID `{}`.\nEl receptor, {}, tiene el ID `{}`.",
    "{}'s id is `{}`.": "ID {} - `{}`.",
    "Your id is `{}`.": "Tu ID - `{}`.",
    ...
    "Try forwarding the following message to me, and you'll see!": "Prueba a enviarme el siguiente mensaje y lo verás!",
    "/save test This is a markdown test. _italics_, *bold*, `code`, [URL](example.com) [button](buttonurl:github.com) [button2](buttonurl://google.com:same)": """/save test Esto es un test de markdown. _cursiva_, *negrita*, `codigo`, \
[URL](example.com) 
...
el caso de los datos mencionados anteriormente.""",
    ...
    "Could not demote. I might not be admin, or the admin status was appointed by another user, so I can't act upon them!": "No puedo quitarle el admin. Puede que no sea administrador o que el estado de administrador fuese dado por otro usuario, asi que no puedo actuar sobre él!",
    ...
    # Antiflood
    "I like to leave the flooding to natural disasters. But you, you were just a disappointment. Get out.": "Suelo tener bastante paciencia con la gente pesada, pero te has pasado. ¡Largo de aquí!",
    "I can't kick people here, give me permissions first! Until then, I'll disable antiflood.": "No puedo expulsar a la gente aquí, dame permisos primero! Hasta que eso ocurra deshabilitaré el antiflood.",
    "Antiflood has been disabled.": "Antiflood ha sido deshabilitado.",
    "Antiflood has to be either 0 (disabled), or a number bigger than 3 (enabled)!": "Antiflood tiene que ser 0 (deshabilitado), o un número superior a 3 (habilitado)!",
    "Antiflood has been updated and set to {}": "Antiflood se ha actualizado y ha sido establecido a {}",
    "Unrecognised argument - please use a number, 'off', or 'no'.": "Comando desconocido- por favor usa un número, 'off', o 'no'.",
    "I'm not currently enforcing flood control!": "Ahora mismo no estoy no controlando el flood!",
    "I'm currently banning users if they send more than {} consecutive messages.": "Estoy baneando a todos los usuarios que envíen más de {} mensajes consecutivos.",
    # Antispam
    "I've enabled antispam security in this group. This will help protect you from spammers, unsavoury characters, and the biggest trolls.": "He activado la seguridad antispam en este grupo. Esto te ayudará a protegerte contra spammers, personas desagradables y trolls.",
    "I've disabled antispam security in this group. GBans wont affect your users anymore. You'll be less protected from any trolls and spammers though!": "He desactivado la seguridad antispam en este grupo. Los Bans Globales no afectarán a los usuarios. Estarás menos protegido de trolls y spammers!",
    "Give me some arguments to choose a setting! on/off, yes/no!\n\nYour current setting is: {}\nWhen True, any gbans that happen will also happen in your group. When False, they won't, leaving you at the possible mercy of spammers.": "Dame algún comando para establecer la configuración! on/off, yes/no!\n\nTu configuración actual es: {}\nCuando sea True, cualquier Ban Global que ocurra tambien ocurrirá en tu grupo. Cuando sea False, los Ban Globales no afectarán en tu grupo, dejandolo a merced de posibles spammers.",
    ...
    "You haven't specified a time to ban this user for!": "No has especificado el tiempo para banear a este usuario!",
    ...
    "Added <code>{}</code> to the blacklist in <b>{}</b>!": "Añadido <code>{}</code> a la lista negra en <b>{}</b>!",
    "Tell me which words you would like to add to the blacklist.": "Dime qué palabras te gustaría añadir a la lista negra.",
    "Removed <code>{}</code> from the blacklist in <b>{}</b>!": "Borrado <code>{}</code> de la lista negra en <b>{}</b>!",
    "This isn't a blacklisted trigger...!": "Este no es un comando de la lista negrа...!",
    "None of these triggers exist, so they weren't removed.": "Ninguno de estos comandos existía, asi que no han sido borrados.",
    "Removed <code>{}</code> triggers from the blacklist in <b>{}</b>! {} did not exist, so were not removed.": "Borrado <code>{}</code> de los comandos de la lista negra en <b>{}</b>! {} no existe, así que no ha sido borrado!.",
    "Tell me which words you would like to remove from the blacklist.": "Dime que palabras te gustaría borrat de la lista negra.",
    # Filters
    "*Filters in {}:*\n": "*Filtros en {}:*\n",
    "local filters": "filtros locales",
    "*local filters:*\n": "*filtros locales:*\n",
    "No filters in {}!": "No ha filtros en {}!",
    "There is no note message - You can't JUST have buttons, you need a message to go with it!": "No hay mensaje - No puedes tener botones vacíos, necesitas un mensaje que vaya dentro del botón!",
    ...
    "That's not a current filter - run /filters for all active filters.": "Actualmente eso no es un filtro - escribe /filters para ver los filtros activos.",
    # Disable
    ...
    "What are you trying to lock...? Try /locktypes for the list of lockables": "¿Qué estás intentando bloquear...? Escribe /locktypes para ver la lista de los tipos de bloqueos.",
    "I'm not an administrator, or haven't got delete rights.": "No soy administrador o no tengo permisos para borrar.",
    "Unlocked {} for everyone!": "Desbloqueado {} para todos!",
    "What are you trying to unlock...? Try /locktypes for the list of lockables": "¿Qué estás intentando desbloquear...? Escribe /locktypes para ver la lista de los tipos de bloqueos.",
    "What are you trying to unlock...?": "¿Qué estás intentando desbloquear...?",
    "I see a bot, and I've been told to stop them joining... but I'm not admin!": "He visto un bot y se me ha ordenado evitar que entre al grupo..., pero no soy administrador!",
    "Only admins are allowed to add bots to this chat! Get outta here.": "¡Solo se permite a los administradores añadir bots! ¡Fuera de aquí!",
    "There are no current locks in *{}*.": "No hay bloqueos en *{}*.",
    "These are the locks in *{}*:": "Estos son los bloqueos en *{}*:",
    "this chat": "este chat",
    # Log channel
    "Now, forward the /setlog to the group you want to tie this channel to!": "Ahora envia /setlog al grupo con el que quieres vincular este canal!",
    "This channel has been set as the log channel for {}.": "Este canal ha sido configurado como el canal de registro para {}.",
 ...
 • envía el /setlog que has puesto en el canal al grupo.""",
    ...
    # Users
    "I've seen them in <code>{}</code> chats in total.": "Le he visto en <code>{}</code> chats en total.",
    "I've seen them in... Wow. Are they stalking me? They're in all the same places I am... oh. It's me.": "Le he visto en...Wow. ¿Me estás siguiendo? Estás los mismos sitios que yo... oh, pero si soy yo! Que chorprecha!!.",
    # Msg_deleting
    "Cannot delete all messages. The messages may be too old, I might not have delete rights, or this might not be a supergroup.": "No he podido borrar todos los mensajes. Puede que los mensajes sean muy viejos, que no tenga derechos para borrarlos o que esto no sea un supergrupo.",
    "Purge complete.": "Purga completada.",
    "Reply to a message to select where to start purging from.": "Responde a un mensaje para seleccionar desde donde empezar la purga.",
    "Whadya want to delete?": "¿Qué quieres borrar?",
    # Muting
    "You'll need to either give me a username to mute, or reply to someone to be muted.": "Necesitas darme un nombre de usuario para silenciar, o responde a alguien para silenciarle.",
    ...
    "You'll need to either give me a username to unmute, or reply to someone to be unmuted.": "Necesitas darme un nombre de usuario para dejar de silenciarle, o responde a alguien que está silenciado para quitarle el silencio.",
    ...
    "This user isn't even in the chat, unmuting them won't make them talk more than they already do!": "Este usuario ni siquiera está en el chat.",
    ...
    "You haven't specified a time to mute this user for!": "¡No has especificado el tiempo para silenciar a este usuario!",
    ...
    "You'll need to either give me a username to restrict, or reply to someone to be restricted.": "Necesitas darme un nombre de usuario para restringir, o responder a alguien para restringirle.",
    ...
    "You'll need to either give me a username to unrestrict, or reply to someone to be unrestricted.": "Necesitas darme un nombre de usuario para quitar la restricción, o responder a un mensaje de esa persona.",
    "This user already has the rights to send anything in {}.": "Este usuario ya tiene permisos para enviar cualquier cosa en {}.",
    "Yep, {} can send media again in {}!": "Si, {} puede volver a enviar media en {}!",
    "This user isn't even in the chat, unrestricting them won't make them send anything than they already do!": "ЭEste usuario ni siquiera está en el chat.",
    ...
    "You haven't specified a time to restrict this user for!": "¡No has especificado el tiempo para restringir a este usuario!",
